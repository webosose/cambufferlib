...
SHMEM_STATUS_T _ReadShmem(SHMEM_HANDLE hShmem, unsigned char **ppData,
                          int *pSize, unsigned char **ppExtraData,
                          int *pExtraSize, int readMode);
SHMEM_STATUS_T _WriteShmem(SHMEM_HANDLE hShmem, unsigned char *pData,
...
SHMEM_STATUS_T CreateShmem(SHMEM_HANDLE *phShmem, key_t *pShmemKey,
                           int unitSize, int unitNum) {
  return _OpenShmem(phShmem, pShmemKey, unitSize, unitNum, 0, MODE_CREATE);
}

SHMEM_STATUS_T CreateShmemEx(SHMEM_HANDLE *phShmem, key_t *pShmemKey,
                             int unitSize, int unitNum, int extraSize) {
  return _OpenShmem(phShmem, pShmemKey, unitSize, unitNum, extraSize,
                    MODE_CREATE);
}
...
SHMEM_STATUS_T ReadShmem(SHMEM_HANDLE hShmem, unsigned char **ppData,
                         int *pSize) {
  return _ReadShmem(hShmem, ppData, pSize, NULL, NULL, READ_FIRST);
}

SHMEM_STATUS_T ReadLastShmem(SHMEM_HANDLE hShmem, unsigned char **ppData,
                             int *pSize) {
  return _ReadShmem(hShmem, ppData, pSize, NULL, NULL, READ_LAST);
}

SHMEM_STATUS_T ReadShmemEx(SHMEM_HANDLE hShmem, unsigned char **ppData,
                           int *pSize, unsigned char **ppExtraData,
                           int *pExtraSize) {
  return _ReadShmem(hShmem, ppData, pSize, ppExtraData, pExtraSize, READ_FIRST);
}

SHMEM_STATUS_T ReadLastShmemEx(SHMEM_HANDLE hShmem, unsigned char **ppData,
                               int *pSize, unsigned char **ppExtraData,
                               int *pExtraSize) {
...
SHMEM_STATUS_T _ReadShmem(SHMEM_HANDLE hShmem, unsigned char **ppData,
  ...
  static bool first_read;
    ...
    DEBUG_PRINT("entered into read shmem\n");
  first_read = false;
  if (!shmem_buffer) {
  ...
  do {
    ...
    if (-1 != *shmem_buffer->write_index) {
      ...
      read_addr =
          shmem_buffer->data_buf + (lread_index) * (*shmem_buffer->unit_size);
      *ppData = read_addr;
      *pSize = size;
...
SHMEM_STATUS_T WriteShmemEx(SHMEM_HANDLE hShmem, unsigned char *pData,
                            int dataSize, unsigned char *pExtraData,
                            int extraDataSize) {
  return _WriteShmem(hShmem, pData, dataSize, pExtraData, extraDataSize);
}

SHMEM_STATUS_T WriteShmem(SHMEM_HANDLE hShmem, unsigned char *pData,
                          int dataSize) {
...
SHMEM_STATUS_T _WriteShmem(SHMEM_HANDLE hShmem, unsigned char *pData,
...
#ifdef SHMEM_COMM_DEBUG
  {
    // int sem_count;
    // sem_count =
    semctl(shmem_buffer->sema_id, 0, GETVAL, 0);
  }
#endif
  ...
  if (lwrite_index == (unit_num - 1)) {
    DEBUG_PRINT(
        "Overflow write data(read index = %d, write_index = %d, unit_num = "
        "%d)!\n",
